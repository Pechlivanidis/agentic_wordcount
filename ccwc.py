...
def main():
    ...
    if option == '-c':
        ...
        # Calculate field width like wc does: use the width of the largest number
        max_width = max(len(str(line_count)), len(str(word_count)), len(str(byte_count)))
        
        print(f"{line_count:>{max_width}} {word_count:>{max_width}} {byte_count:>{max_width}} {filename}")
    else:
